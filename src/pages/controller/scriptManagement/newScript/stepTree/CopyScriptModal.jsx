            ...
            render: (text, record, index) => {
                return (
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {record['case_type'] == '用户脚本' && <Tag color='blue'>用户脚本</Tag>}
    ...
    const handleSearch = ({ page, size }) => {
        ...
        dispatch({
            ...
            params: {
                ...
                case_type: form.getFieldValue('caseType') ? [form.getFieldValue('caseType')] : ['公共脚本', '公共接口', '用户脚本'],
                page,
                page_size: size
            }
        })
    ...
    const handleOK = () => {
        if (selectedRows.length > 0) {
            if (caseInfo?.case_id == selectedRowKeys[0]) {
                message.error('脚本复制不能选择自身，请选择其他脚本');
                return;
            }

            setLoading(true);
            dispatch({
                type: 'scriptManagement/StepCopyTreeSearch',
                params: {
                    ...
                    <Col span={6}>
                        <Form.Item labelCol={{ span: 8 }} wrapperCol={{ span: 15 }} label="类型" name="caseType" >
                            ...
                            >
                                ...
                                <Option key="用户脚本" value="用户脚本">用户脚本</Option>
                            </Select>
                        </Form.Item>
                    </Col>
                    <Col span={8}>
