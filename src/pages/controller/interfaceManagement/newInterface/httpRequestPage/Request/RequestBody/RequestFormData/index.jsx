...
const RequestFormData = (props) => {
    ...
    useEffect(() => {
        if (interfaceInfo?.request_form_data && interfaceInfo?.request_form_data.length > 0) {
            setDataSource(transformData(interfaceInfo?.request_form_data));
        } else {
            setDataSource([]);
            addNewRow();
        }
    ...
    const deleteRow = (id) => {
        setDataSource(prev => {
            ...
            updateInterfaceInfo('request_form_data', newList);
            return newList;
        });
    };
