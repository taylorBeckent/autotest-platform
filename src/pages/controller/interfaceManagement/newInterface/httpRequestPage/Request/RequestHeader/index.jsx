...
import { Table, Input, Button, Space, Card, AutoComplete, Tooltip, Tag, message, List, Tabs, Menu, Modal } from 'antd';
...
const RequestHeader = (props) => {
    ...
    const {
        dispatch,
        scriptManagement: { stepTreeList, variableList },
    ...
    useEffect(() => {
        if (stepTreeList && stepTreeList.length > 1 && stepTreeList[1]?.request_header && stepTreeList[1]?.request_header.length > 0) { //. 有默认值
            let newData = transformData(stepTreeList[1]?.request_header);
        ...
        let variableOptions = renderVariables();
    ...
    const extractAllVariables = () => {
        let interfaceInfoCopy = JSON.parse(JSON.stringify(interfaceInfo));
        let tempList = [];

        if (interfaceInfoCopy.extract_variables && interfaceInfoCopy.extract_variables.length > 0) {
            interfaceInfoCopy.extract_variables.forEach(item => {
                if (item.name) {
                    let obj = {};
                    obj.key = item.name;
                    obj.value = '${' + item.name + '}';
    ...
    const deleteRow = (id) => {
        setDataSource(prev => {
            ...
            updateInterfaceInfo('request_header', newList);
            return newList;
        });
    };
    ...
    const editContent = (
        <div>
            <Card title="批量导入" size="small" extra={<CloseOutlined style={{ cursor: 'pointer', fontSize: 16, color: "#8b8b8b" }} onClick={() => { setBatchStatus(false) }} />} >
                <TextArea
                    style={{ width: '100%' }}
                ...
                footer={() => (
                    <div style={{ display: 'flex' }} >
                        ...
                        <Tooltip placement="bottom" arrowPointAtCenter={true} trigger="click" title={editContent}
                            ...
                            open={batchStatus}
                        >
                            <Button type="dashed" onClick={openAndCloseBatchModal} block>
                                + 从剪切板导入
                            </Button>
                        </Tooltip>
                    </div>
                    ...
                    <div className={styles['variable-content']}>
                        ...
                        <div className={styles['variable-List']} >
                            <Tabs defaultActiveKey="commonFun" tabPosition="left" >
                                <TabPane tab="接口全局变量" key="scriptVariable">
