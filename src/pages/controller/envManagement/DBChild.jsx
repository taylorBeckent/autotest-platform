...
import { Table, Button, Popover, Popconfirm, message } from 'antd';
...
const DBChildTable = (props) => {
            ...
            render: (text, record) => {
                return (
                    <Button.Group className={styles.btchBtn}>
                        ...
                        <Popover content='测试链接'>
                            <Button icon={<LinkOutlined />} type="link" onClick={() => { handleTestConnect(record) }} />
                        </Popover>
                    </Button.Group >
                )
            }
    ...
    //. 测试连接
    const handleTestConnect = (record) => {
        setChildLoading(true);
        dispatch({
